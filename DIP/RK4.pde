    ...
    for (int j = 0; j<4; j++)
      for (int i = 0; i<6; i++)  //iterating through system of ODE's
      {
        ...
        if (j>2)      //fourth step of RK4
          d=1;
        if (!controled_object)
          k[i][j] = h * f1(i, x[1] + (z ? (k[1][j-1]/d) : 0), x[2] + (z ? (k[2][j-1]/d) : 0), x[3] + (z ? (k[3][j-1]/d) : 0), x[4] + (z ? (k[4][j-1]/d) : 0), x[5] + (z ? (k[5][j-1]/d) : 0), u, z0, z1, z2); //RK4 main equation, modified to system of ODE's
        else
